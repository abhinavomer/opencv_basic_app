...
if op=='Home':
    ...
    st.image("proj.jpg")
    st.markdown("The OpenCV app presented here is a versatile tool for image processing and live video filtering. It offers a range of features accessible through a user-friendly interface built with Streamlit. Users can choose from various image filters like face and eye detection, edge detection, cartoonification, blurring, image resizing, and QR code scanning. Additionally, the app provides live video filters, including live camera streaming, real-time face and eye detection, Canny edge detection, blur filtering, and feature highlighting. These features leverage OpenCV's powerful computer vision algorithms and Streamlit's interactive capabilities to deliver an intuitive and engaging experience for users interested in exploring image processing techniques and experimenting with real-time video effects. Despite facing some memory limitations in the deployment environment, the app demonstrates the potential of combining OpenCV's image processing capabilities with Streamlit's simplicity and flexibility for creating interactive applications.")
if op=='Image_Filters':
